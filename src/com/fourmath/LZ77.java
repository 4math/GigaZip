...
public class LZ77 {
    ...
    public int lookAheadBufferSize = 128; // 128 max
    ...
    public byte[] encode(byte[] input) throws IOException {
        ...
        while (cursor < end) {
            ...
            for (int pos = 0; pos < suff.length; pos++) {
                ...
                if (suff[pos].index + start == cursor) {
                    if (pos - 1 >= 0 && suff[pos - 1].index + start < cursor) {
                        left = lcp[pos - 1];
                    }

                    if (pos + 1 < lcp.length && suff[pos + 1].index + start < cursor) {
                        right = lcp[pos + 1];
                    }

                    if (left == -1 && right == -1) {
                        updateUniquePtr(output, input, cursor);
//                            System.out.println((cursor + 1) + " One symbol");
                        bfw.write((cursor + 1) + " One symbol");
                        bfw.write("\n");
                        cursor++;
                        break;
                    }
...
//                                System.out.println((cursor + 1) + " One symbol");
                        ...
                        bestOffset = cursor - (suff[pos + 1].index + start);
                        bestMatchLength = right;
                        if (bestMatchLength > lookAheadBufferSize) {
                            bestMatchLength = lookAheadBufferSize;
                        }
//                            System.out.println("Match: " + (cursor + 1) + " length " + bestMatchLength + " offset " + bestOffset);
                        bfw.write("Match: " + (cursor + 1) + " length " + bestMatchLength + " offset " + bestOffset);
                        bfw.write("\n");
...
//                            bestOffset = cursor - suff[pos - 1].index;
...
//            cursor += lookAheadBufferSize;
            ...
            if (cursor >= end) {
                ...
                end = Math.min(lookAheadBufferEndPtr, input.length);
                start = Math.max(searchBufferStartPtr, 0);
                bsa = new ByteSuffixArray(start, end, input);
                lcp = bsa.getLcp();
                suff = bsa.getSuff();
            }
//            cursor++;
